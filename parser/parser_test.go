...
func TestBooleanExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("exp not converted to boolean. got=%T", stmt.Expression)
	}
	if boolean.Value != true {
		t.Errorf("ident.Value not %t. got=%t", true, boolean.Value)
	}
...
func TestIfExpression(t *testing.T) {
	...
	if len(exp.True.Statements) != 1 {
		t.Errorf("consequence is not 1 statements. got=%d\n",
			len(exp.True.Statements))
	}
	consequence, ok := exp.True.Statements[0].(*ast.ExpressionStatement)
	if !ok {
		t.Fatalf("Statements[0] is not ast.ExpressionStatement. got=%T",
			exp.True.Statements[0])
	}
	if !testIdentifier(t, consequence.Expression, "x") {
		return
	}
	if exp.False != nil {
		t.Errorf("exp.Alternative.Statements was not nil. got=%+v", exp.False)
	}
}
...
func TestIfElseExpression(t *testing.T) {
	...
	if len(exp.False.Statements) != 1 {
		t.Errorf("exp.Alternative.Statements does not contain 1 statements. got=%d\n",
			len(exp.False.Statements))
	}

	alternative, ok := exp.False.Statements[0].(*ast.ExpressionStatement)
	if !ok {
		t.Fatalf("Statements[0] is not ast.ExpressionStatement. got=%T",
			exp.False.Statements[0])
	}
